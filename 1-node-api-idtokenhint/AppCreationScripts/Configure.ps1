...
#> 
    ...
    Write-Host "Creating the AAD application (Verifiable Credentials VC Node sample)"
    $clientAadApplication = New-AzADApplication -DisplayName "Verifiable Credentials VC Node sample" `
                                                -IdentifierUris "https://$tenantDomainName/vcnodesample1" 
    ...
    Add-Content -Value "<tr><td>client</td><td>$($clientAadApplication.AppId)</td><td><a href='$clientPortalUrl'>VC Node sample</a></td></tr>" -Path createdApps.html

    # Update config file for 'client'
    $dictionary = @{ "<YOUR-AAD-TENANDID>" =  $tenantId; "<YOUR-AAD-CLIENTID-FOR-API-ACCESS>" = $clientAadApplication.AppId; "<YOUR-AAD-CLIENTSECRET-FOR-API-ACCESS>" = $appCreds.SecretText };
